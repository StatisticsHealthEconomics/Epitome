...
:::: {.column width="45%"}
- [Layer 1 -- Welfare Status]{.samp-red}: a binary classification of either [Welfare Entry (active claim; 6)]{.light-blue} or [Welfare Exit (7)]{.orange}

- [Layer 2 -- Well-Being Space]{.samp-blue}: five distinct states of human experience:
...
\logit({\chi}_{q',q}^{i,t}) = {{\eta}_{q',q}^{i,t}} = & {\color{olive}\beta_{0}^{q}} + {\color{red}\beta_{1}^{q}}\txt{Age}^{i,t} +{\color{red}\beta_{2}^{q}} \txt{HEQ}^{i,t}+ {\color{red}\beta_{3}^{q}}\txt{ETH}^{i,t} + {\color{red}\beta_{4}^{q}}\txt{MS}^{i,t} + {\color{red}\beta_{5}^{q}}\txt{Sex}^{i,t} + {\color{red}\beta_{6}^{q}}\txt{GR}^{i,t} + \\
...
## Model: Bayesian Multinomial-logistic ITS {visibility="uncounted"}
...
\logit\left({{\lambda}_{s',s}^{i,t}}(q) \right) = {{\eta}_{s',s}^{i,t}}(q) = & {\color{olive}\beta_{0}^{s}} + {\color{red}\beta_{1}^{s}}\txt{Age}^{i,t} + {\color{red}\beta_{2}^{s}} \txt{HEQ}^{i,t} + {\color{red}\beta_{3}^{s}}\txt{ETH}^{i,t} + {\color{red}\beta_{4}^{s}}\txt{MS}^{i,t} + {\color{red}\beta_{5}^{s}}\txt{Sex}^{i,t} + {\color{red}\beta_{6}^{s}\txt{GR}^{i,t}} + \\
...
- Structured terms
  ...
  - [Random effects to account for temporal variability and individual heterogeneity]{.purple}
  - [Cubic B-splines]{.orange} to map complex, non-linear population changes across the 2009-2020 timeline
  - [Intervention effect]{.samp-red}: shifts the log-odds of a well-being transition if the individual is currently receiving benefits (= in state $q=7$)
...
ggplot(d, aes(x = year_dodge, y = mean, colour = benefit, group = benefit)) +
  ...
  scale_colour_manual(values = cols, name = "Benefits") +
  scale_x_continuous(breaks = 2010:2020) +
  labs(title = "WE", x = "Year", y = "Proportion of Individuals") +
  theme_ucl() +
  theme(
...
BCEA::contour(m,graph="ggplot2") + theme_ucl() + theme(legend.position="none") 
```
