...
- I can take all the credit (:wink:), but in reality, this has been driven by the fantastic work of **Ioannis Rotous**
...
- The [**Legacy Welfare**]{.samp-blue} (LW) system is a complex, post-WWII evolved safety net that often required multiple, non-integrated applications for different areas of support
- The 2010s policy shift aimed to replace this administrative complexity with [**Universal Credit**]{.samp-red} (UC) -- a single, integrated payment structure
- Primary Policy Aims:
...
- $\lambda_{44} =1$. Assumes no zombies. . .
- $\ldots$
...
## Bayesian Multinomial-logistic ITS: $w$elfare status {#model}
...
- Structured terms
  ...
  - [[[Cubic B-splines](#model-specification)]{.orange}]{.no-link} to map complex, non-linear population changes across the 2009-2020 timeline

## Bayesian Multinomial-logistic ITS: $w$ell $b$eing status {#model2 visibility="uncounted"}
...
- Use [[minimally informative priors](#prior-distributions)]{.no-link} + [[Variational Inference](#variational-inference)]{.no-link} to generate 1000 joint posterior samples
...
- Adapts standard Health Technology Assessment (HTA) principles to measure the financial cost per unit of human well-being achieved by each system
  ...
  - Both [[discounted](#discounting)]{.no-link} at 3.5% yearly, in line with NICE suggestions
...
# Back up slides {.divider visibility="uncounted"}

## Model specification {visibility="uncounted"}

[
$$
\begin{aligned}
f^{h}_{\txt{Exposed}^{i}}(t)  = & \sum_{m=1}^{M}B_{m}(t)\left(\alpha_{m,\txt{NB}}^{h} + \alpha_{m,\txt{LW}}^{h}\mathbb{I}(\txt{Exposed}^{i}=\txt{LW}) + \alpha_{m,\txt{UC}}^{h}\mathbb{I}(\txt{Exposed}^{i}=\txt{UC})\right)
\end{aligned}
$$]{.myblue}

- $h$ indicates the state in the Markov model
- $M$ is the total number of basis functions -- set to $M=7$
- $B_m(t)$ is the $m$-th basis function at time $t$. These are cubic splines -- capture non-linearity
- $\alpha_{m,\txt{NB}}^{h}, \alpha_{m,\txt{LW}}^{h}, \alpha_{m,\txt{UC}}^{h} \stackrel{iid}{\sim} \dnorm(0,v)$ are the spline coefficients

`r vspace("20px")`

[
$$
f^{h}_{\txt{Intervention}^{i,t}} = \sum_{m=1}^{M} B_{m}(t) \left( \omega_{m,\txt{Mig}}^{h} \txt{Intervention}_{\txt{Mig},i,t} + \omega_{m,\txt{New}}^{h} \txt{Intervention}_{\txt{New},i,t} \right)
$$]{.myblue}

- $\txt{Intervention}_{\txt{Mig},i,t}$ is a binary indicator for individuals who transitioned from LW to UC
- $\txt{Intervention}_{\txt{New},i,t}$ is a binary indicator for individuals who initiated a new UC claim from the NB population
- $\omega_{m,\txt{Mig}}^{h}, \omega_{m,\txt{New}}^{h} \stackrel{iid}{\sim} \dnorm(0,v)$ are the spline coefficients


{{< fa arrow-left >}} [Back to slides](#model)

## Prior distributions {visibility="uncounted"}

- Unstructured effects
   - $\bm\beta^q, \bm\beta^s, \theta^s \stackrel{iid}{\sim} \dnorm(0,v)$
- Individual heterogeneity
   - $e_{i}^{s} \sim \dnorm(0,\sigma_{e}^{s})$ and $e_{i}^{ q} \sim \text{Normal}(0, \sigma_{e}^{q})$, with $\sigma_{e}^{s}, \sigma_{e}^{q} \stackrel{iid}{\sim} \txt{Half-Normal}(0, v)$
- Random effect by PSU
   - $\zeta_{\psi_{i}}^{s} \sim \dnorm(0,\sigma_{\zeta}^{s})$ and $\zeta_{\psi_{i}}^{q} \sim \text{Normal}(0, \sigma_{\zeta}^{q})$, with $\sigma_{\zeta}^{s}, \sigma_{\zeta}^{q} \stackrel{iid}{\sim} \txt{Half-Normal}(0, v)$
- Random effect by stratification group
   - $\nu_{k_{i}}^{s} \sim \dnorm(0,\sigma_{\nu}^{s})$ and $\nu_{k_{i}}^{q} \sim \text{Normal}(0, \sigma_{\nu}^{q})$, with $\sigma_{\nu}^{s}, \sigma_{\nu}^{q} \stackrel{iid}{\sim} \txt{Half-Normal}(0, v)$
- Linear trend
   - $\delta_t^s \sim \dnorm(0,\sigma_{\delta}^{s})$, with $\sigma_{\delta}^{s} \sim \txt{Half-Normal}(0,v)$
- LSOA spatial effects
   - $\gamma_{r_{i}}^{s} \sim \txt{BYM2} \Rightarrow$ specifically designed to account for spatial correlation

{{< fa arrow-left >}} [Back to slides](#model2)

## Variational inference {visibility="uncounted"}
...
- Variational inference (VI) turns inference into optimisation
   - Pick a simple family $q_\phi(\theta)$ and find $\phi$ minimising $\txt{KL}\left(q_\phi \mid\mid p(\theta \mid y)\right)$
   [
   $$
   \begin{aligned}
   \txt{KL}\left(q_\phi \mid\mid p(\theta \mid y)\right) =& \txt{E}_{q_\phi}\left[\log q_\phi(\theta) - \log p(\theta \mid y)\right] \\
   =& \txt{E}_{q_\phi}\left[\log q_\phi(\theta) - \log p(\theta)p(y \mid \theta) + \log p(y)\right] \\
   =& \underbrace{\txt{E}_{q_\phi}\left[\log q_\phi(\theta) - \log p(\theta)p(y \mid \theta)\right]}_{-\txt{Evidence Lower BOund}(\phi)} + \log p(y) \\[5pt]
   \log p(y) = & \txt{ELBO}(\phi) + \txt{KL}\left(q_\phi(\theta) \mid\mid p(\theta \mid y)\right)
   \end{aligned}
   $$]{.myblue}
...
## Discounting {visibility="uncounted"}
...
d=0.03
t=seq(0,15)
tibble(t=t,value=x/((1+d)^t)) |> ggplot(aes(t,value)) + geom_col(width=.8) +
  geom_text(aes(label = round(value, 1)), vjust = -0.3) +
  xlab("Time (years)") + ylab("Discounted value")
```
:::

::: {.column width="50%"}
- When discounted at 3%, something that has a value of 100 today (time $t=0$) has a value of 97.1 at time $t=1$, 94.3 at time $t=2$, etc

- In general:
[
$$
\txt{Present value} = \sum_{t=0}^T \frac{\txt{Value}}{(1+\txt{Discount rate})^t}
$$
]{.myblue}
:::
::::
...
{{< fa arrow-left >}} [Back to slides](#economic-evaluation)